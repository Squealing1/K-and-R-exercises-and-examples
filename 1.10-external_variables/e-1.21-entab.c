...
void entab(char line[], char et_line[], int tabstop);
...
void entab(char line[], char et_line[], int tabstop){
  int str_i = 0; // original string index
  int et_s_i = 0; // entabed string index
  int spaces = 0; // amount of spaces to potentially be replaced by tab or prefixed to character
  int tabstop_i = 0; // spot in tab interval
  while ( line[str_i] != '\n' ) {
    if (tabstop_i % tabstop == 0) { // Reached tabstop
      if ( spaces > 0 ) {
        et_line[et_s_i] = '\t';
        et_s_i++;
        spaces = 0;
    ...
    if ( line[str_i] == ' ' ) { // Count spaces
      spaces++;
    }
    else if ( line[str_i] == '\t' ) { // Erase spaces and reset tabstop interval
      et_line[et_s_i] = '\t';
      et_s_i++;
      spaces = 0;
      tabstop_i = 0;
      ...
      // prefix spaces
      for ( int space = 0; space < spaces; space++ ) { et_line[et_s_i] = ' '; et_s_i++; }
      spaces = 0;

      // match character
      et_line[et_s_i] = line[str_i];
      et_s_i++;
    }
    str_i++;
  }
